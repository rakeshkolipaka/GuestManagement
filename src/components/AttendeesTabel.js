...
function AttendeesTabel({ attendee, index, changeStatus, setSendPdfModalIsOpenToTrue }) {
	return (
		<div
			...
			style={{borderLeft: attendee.isEntered? "10px solid #6c757d" : "10px solid #156F35"}}
		>
			{attendee.name}
			

			<div>
				<button style={{display: attendee.isEntered? "none": "block", backgroundColor: "#156F35"}} onClick={() => changeStatus(index)}><BsCheck2/>  Here</button>
